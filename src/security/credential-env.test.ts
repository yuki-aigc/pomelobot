...
test('credential env file is loaded and process env takes priority', async () => {
    const tempDir = await mkdtemp(join(tmpdir(), 'pomeloclaw-cred-env-'));
    const envFile = join(tempDir, '.env');
    const originalPath = process.env.POMELOCLAW_CREDENTIALS_ENV_PATH;
    ...
    try {
        ...
        process.env.POMELOCLAW_CREDENTIALS_ENV_PATH = envFile;
        ...
        if (originalPath === undefined) {
            delete process.env.POMELOCLAW_CREDENTIALS_ENV_PATH;
        } else {
            process.env.POMELOCLAW_CREDENTIALS_ENV_PATH = originalPath;
        }
...
test('withTemporaryCredentialEnv injects missing vars and restores previous values', async () => {
    const tempDir = await mkdtemp(join(tmpdir(), 'pomeloclaw-cred-inject-'));
    const envFile = join(tempDir, '.env');
    const originalPath = process.env.POMELOCLAW_CREDENTIALS_ENV_PATH;
