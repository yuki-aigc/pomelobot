...
test('credential env file is loaded and process env takes priority', async () => {
    const tempDir = await mkdtemp(join(tmpdir(), 'pomelobot-cred-env-'));
    const envFile = join(tempDir, '.env');
    const originalPath = process.env.POMELOBOT_CREDENTIALS_ENV_PATH;
    ...
    try {
        ...
        process.env.POMELOBOT_CREDENTIALS_ENV_PATH = envFile;
        ...
        if (originalPath === undefined) {
            delete process.env.POMELOBOT_CREDENTIALS_ENV_PATH;
        } else {
            process.env.POMELOBOT_CREDENTIALS_ENV_PATH = originalPath;
        }
...
test('withTemporaryCredentialEnv injects missing vars and restores previous values', async () => {
    const tempDir = await mkdtemp(join(tmpdir(), 'pomelobot-cred-inject-'));
    const envFile = join(tempDir, '.env');
    const originalPath = process.env.POMELOBOT_CREDENTIALS_ENV_PATH;
