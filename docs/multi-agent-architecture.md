# 多 Agent / 多 Session 设计（Pomelobot）

> 本文档定义 Pomelobot 的多 agent 演进方向。目标是把当前“单主 agent + 轻量 subagent”的结构，升级为“通用协作框架 + 可插拔专家包”，同时保持当前项目的渠道、记忆、MCP、审批、审计能力不被破坏。
...
> Pomelobot 应该从“单 SRE agent”演进为“团队助手框架”，SRE 只是第一套可插拔专家包。
