...
- PG 可用时（`backend=pgsql`）：
  ...
  - `dingtalk_session_events`：会话事件（session 热日志）
- PG 不可用时：
...
- path 以 `session_events/` 开头：
  - 读取 PG `dingtalk_session_events` 单条事件（按 `session_key + conversation_id + event_id`）
- 其他 path：
...
- 维度不匹配时会清理/跳过异常缓存
- `dingtalk_session_events` 的 embedding 采用异步回填（先写事件，后补向量），避免阻塞消息写入链路
...
2. `dingtalk_session_events` 检索（会话热日志）
...
1. 高优先：给 `dingtalk_session_events` 增加持久化向量列 + ANN 索引
