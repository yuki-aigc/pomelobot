# Memory 机制说明（Pomeloclaw）

> 本文档说明 Pomeloclaw 的记忆机制、检索机制、会话机制与运维要点。
...
- 主会话长期记忆：`workspace/memory/scopes/main/MEMORY.md`
- 主会话每日记忆：`workspace/memory/scopes/main/YYYY-MM-DD.md`
- 隔离 scope 长期记忆：`workspace/memory/scopes/<scope>/MEMORY.md`
...
Agent 调用 `memory_save_team` 时：
- 固定写入 `main` scope（团队共享记忆）
- `target=daily` -> `workspace/memory/scopes/main/YYYY-MM-DD.md`
- `target=long-term` -> `workspace/memory/scopes/main/MEMORY.md`
- 建议使用结构化字段产出固定条目：标题、摘要、适用场景、操作步骤、边界与注意事项、证据与依据、标签
- 团队记忆会按“标题”做去重/合并：同标题再次晋升时，不会追加新条目，而是合并到原条目并更新时间
- 若当前处于隔离 scope，会自动附带来源 scope 注记，便于追溯知识来源
...
为兼顾“会话冷启动记忆”与 token 成本，DingTalk / Web 首轮会话都支持注入今天/昨天的 Markdown 摘要（仅文件层，不读向量库）：

- 只在会话首轮触发（避免每轮重复注入）。
- 仅注入 daily Markdown（统一读取 `memory/scopes/<scope>/YYYY-MM-DD.md`）。
...
- 归档完成后，任务会进一步判断是否存在可跨会话复用的团队知识；若存在，会额外调用 `memory_save_team` 晋升到团队共享记忆。
...
- 当内容应被沉淀为跨群聊/私聊共享的团队经验、标准流程、通用排障知识时，应调用 `memory_save_team` 晋升到团队记忆。
...
2. DingTalk / Web 通道强制触发（已实现）
- 当用户问题命中回溯意图（如“之前/上次/昨天/问过/聊过/历史”）时，会注入强约束提示，要求先 `memory_search` 再回答。
- 典型日志：`[DingTalk] Memory recall intent detected, enforce memory_search preflight` / `[Web] Memory recall intent detected, enforce memory_search preflight`
...
- 当 `shared_main_scope_reads=true` 时，隔离 scope 在检索阶段会额外读取 `main` 的团队记忆文件（`memory/scopes/main/MEMORY.md`、`memory/scopes/main/YYYY-MM-DD.md`、`memory/scopes/main/HEARTBEAT.md`）。
- 该开关只影响“读”，不影响“写”；群聊/私聊的 `memory_save`、transcript、session events 仍写回各自 scope。
...
- Pomeloclaw：也是“文件 + 索引”模式，但额外引入了 `dingtalk_sessions` / `session_events` 的会话态数据库层。

2. 会话记忆能力
- OpenClaw：文档提到有 experimental session memory 方向。
- Pomeloclaw：DingTalk 侧已经把 session events 纳入统一检索入口，实用性更强。

3. 工具面
- OpenClaw：文档强调 `memory_search` / `memory_get` / `memory_save` 组合。
- Pomeloclaw：已补齐 `memory_search` + `memory_get` + `memory_save` 三件套，并额外支持 session events 路径精读。

4. 同步策略
- OpenClaw：强调后台 watcher + debounce 的异步索引同步。
- Pomeloclaw：采用“启动基线 + 事件触发 + 检索前条件同步”的组合策略，更偏工程可控。
